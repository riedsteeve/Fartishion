...
const links = [
  { id: 1, name: "Accueil", href: "/dashboard" },
  { id: 2, name: "Partitions", href: "/partitions" },
  { id: 3, name: "Ma bibliothèque", href: "/bibliothèque" },
];
        ...
        <ul className="hidden items-center gap-6 lg:flex">
          {links.map((link) => (
            <li key={link.id}>
             ...
             <Link to={"/profil"} className="hidden items-center gap-2 rounded-full border border-purple-600 px-4 py-2 text-purple-600 transition hover:bg-purple-600 hover:text-white lg:flex">
                 <div className="">
                     <img className="w-7 h-7 rounded-full" src={steevepic} alt="Avatar" />
                 </div>
          Prenom.nom
        </Link>
